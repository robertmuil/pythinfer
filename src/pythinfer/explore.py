...
class CompareResult:
    ...
    right_count: int
    only_left: set[tuple]
    only_right: set[tuple]
    both: set[tuple]
    union: set[tuple]
    namespaces: dict[str, str]


def load_graph(path: str | Path) -> set[tuple]:
    """Load an RDF file and return its triples as a set."""
    g = Graph()
    g.parse(str(path))
    return set(g)


def format_triples(triples: set[tuple], namespaces: dict[str, str]) -> list[str]:
    """Format triples as readable strings, applying prefix compression."""
    lines = []
    for s, p, o in sorted(triples, key=str):
        s_str = _shorten(str(s), namespaces)
        p_str = _shorten(str(p), namespaces)
        o_str = _shorten(str(o), namespaces)
...
def collect_namespaces(*paths: str | Path) -> dict[str, str]:
    """Collect namespace prefixes from all input files."""
    ns = {}
    for path in paths:
        g = Graph()
        g.parse(str(path))
...
def compare_graphs(left_path: str | Path, right_path: str | Path) -> CompareResult:
    ...
    namespaces = collect_namespaces(left_path, right_path)
...
def build_interactive_views(
    ...
    views = {
        "left": (f"Only in LEFT ({left_name}): {len(result.only_left)} triples", result.only_left),
        "right": (f"Only in RIGHT ({right_name}): {len(result.only_right)} triples", result.only_right),
        "both": (f"Intersection (both): {len(result.both)} triples", result.both),
        "union": (f"Union (all): {len(result.union)} triples", result.union),
    }
    formatted = {}
    for key, (title, triples) in views.items():
        formatted[key] = (title, format_triples(triples, result.namespaces))
    return formatted


def interactive(stdscr, views: dict[str, tuple[str, list[str]]]) -> None:
